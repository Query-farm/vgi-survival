...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-survival
...
# ATTACH (`${VGI_SURVIVAL_WORKER}`):
#
#   subprocess : a bare stdio command (`uv run survival_worker.py`) — the
...
#                        `uv run --python 3.13 <repo>/survival_worker.py`.
# Optional:
...
WORKER_CMD="${WORKER_CMD:-uv run --python 3.13 $REPO/survival_worker.py}"
...
# ---------------------------------------------------------------------------
# Per-transport: resolve VGI_SURVIVAL_WORKER (the LOCATION) and, for the
...
case "$TRANSPORT" in
  subprocess)
    export VGI_SURVIVAL_WORKER="$WORKER_CMD"
    ;;
  ...
  http)
    ...
    # extra (waitress); WORKER_CMD must resolve it — CI passes `--extra http`
    # and the PEP 723 header lists the extra too.
    PORT_FILE="$(mktemp -u "${TMPDIR:-/tmp}/survival-port.XXXXXX")"
    LOG_FILE="${TMPDIR:-/tmp}/survival-http-server.log"
    ...
    export VGI_SURVIVAL_WORKER="http://127.0.0.1:$PORT"
    ;;
  ...
  unix)
    ...
    SOCK="${TMPDIR:-/tmp}/survival-$$.sock"
    rm -f "$SOCK"
    LOG_FILE="${TMPDIR:-/tmp}/survival-unix-server.log"
    ...
    export VGI_SURVIVAL_WORKER="unix://$SOCK"
    ;;
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_SURVIVAL_WORKER) ..."
